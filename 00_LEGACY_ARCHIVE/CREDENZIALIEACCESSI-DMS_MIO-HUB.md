...
**Ultimo Aggiornamento:** 28 Novembre 2024 - 23:50 CET  
**Stato:** Documento Ufficiale  
**⚠️ ATTENZIONE:** Questo documento contiene informazioni sensibili. Proteggere adeguatamente.

---

## 1. BACKEND MERCATI & GIS (PRODUZIONE)

**⚠️ IMPORTANTE:** Questo è il backend UFFICIALE usato dalla Dashboard PA per:
- Gestione Mercati
- Mappa Grosseto
- Posteggi/Banchi
- Imprese/Concessioni

**La Dashboard PA usa QUESTO backend, NON l'API MIO-hub.**
...
### Deployment su Hetzner

```
Server IP: 157.90.29.66
Path: /root/mihub-backend-rest
URL Base API: https://mihub.157-90-29-66.nip.io/api
...
|----------|-------------|--------|
| `/api/markets/:marketId/stalls` | Lista banchi per mercato | GET |
| `/api/gis/market-map` | Mappa mercati per Editor v3 | GET |
| `/api/markets/:marketId/companies` | Aziende per mercato | GET |
| `/api/markets/:marketId/concessions` | Concessioni per mercato | GET |
| `/api/markets/list` | Lista tutti i mercati | GET |
| `/api/dmsHub/status` | Health check | GET |
| `/api/gis/health` | Health check GIS | GET |

### Status Attuale (28/11/2024)

```
PM2 Status: ❌ errored
Problema: Missing package '@shared/const'
Endpoint: ❌ 502 Bad Gateway
Richiede: Fix monorepo configuration
```

---

## 2. SERVER HETZNER (Infrastruttura)
